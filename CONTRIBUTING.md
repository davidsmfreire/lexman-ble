...
even if you don't use Visual Studio Code. With this container you will have a stand alone
Home Assistant instance running and already configured with the included
[`.devcontainer/configuration.yaml`](./.devcontainer/configuration.yaml)
file.

To use it, run `make dev-container-run` and to stop and remove it `make dev-container-rm`.
...
<!-- TODO: You should also verify that existing [tests](./tests) are still working
and you are encouraged to add new ones.
You can run the tests using the following commands from the root folder:

```bash
# Install requirements (must have poetry installed)
poetry install
# Run tests and get a summary of successes/failures and code coverage
pytest --durations=10 --cov-report term-missing --cov=custom_components.lexman_ble tests
```
